...
# --------------------------------------------------------------------------- glue
# Everything below is NEW glue (factory + checkpoint resolution), not vendored.

_RANDOM_NAMES = (None, "random", "beats_random")


def _resolve_tokenizer_ckpt(spec: str) -> str:
    """Return a local path to a `{"cfg","model"}` tokenizer checkpoint.

    Accepts a local file, a local directory, or a Hugging Face repo id (downloaded
    ...
    if ckpt is None:
        raise FileNotFoundError(f"No tokenizer checkpoint found from '{spec}'.")
    return ckpt
...
def build_tokenizer(
    ...
    *,
    ...
    """Construct a frozen tokenizer for stage-A dumping.

    spec: ``None``/``"random"``/``"beats_random"`` -> seed-deterministic
    ``BeatsRandomTokenizer`` (no checkpoint); otherwise a local checkpoint path,
    local dir, or HF repo id loaded into a real ``BeatsTokenizer``. Returns the
    module on ``device`` in eval mode.
    """
    if spec in _RANDOM_NAMES:
        cfg = dict(tokenizer_config or {})
        cfg.setdefault("seed", seed)
        # The random-projection quantizer's projection/codebook are built from the
        # ambient RNG during __init__ (before BeatsRandomTokenizer._initialize
        # re-seeds for the patch embedding), so seed here too to make the *whole*
        # tokenizer reproducible across constructions/runs.
        torch.manual_seed(cfg["seed"])
        tok = BeatsRandomTokenizer(
            cfg, fbank_mean=fbank_mean, fbank_std=fbank_std
        )
    else:
        ckpt = _resolve_tokenizer_ckpt(spec)
        tok = BeatsTokenizer(
            beats_tokenizer_ckpt_path=ckpt,
