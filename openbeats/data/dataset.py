...
A dataset is ``<dir>/shard-*.parquet``. Each row is one utterance:

    id         : string        utterance id
    audio      : string        absolute audio path (waveform recomputed at train time)
    n_samples  : int64         waveform length (for length-bucketing)
...
Dataset-level config (codebook size, frontend, tokenizer, code offset) is stored in
every shard's Parquet key-value metadata, so each shard is self-describing; a
top-level ``dataset.json`` mirrors it for eyeballing. This module has no torch
dependency on purpose (pure pyarrow/numpy) so it stays light.
"""
...
def build_meta(
    ...
    return {
        ...
        "frontend": dict(frontend or DEFAULT_FRONTEND),
...
class TokenDatasetWriter:
    ...
    def add(self, id: str, audio: str, n_samples: int, codes) -> None:
    ...
    def flush(self) -> Optional[str]:
        ...
        self._ids, self._audios, self._n_samples, self._codes = [], [], [], []
        return path
